...
export default function CompanyInfoPage() {
  ...
  useEffect(() => {
    async function fetchCompany() {
      ...
      const { data, error: fetchError } = await supabase
        .from("companies")
        .select([...FIELDS, "business_categories(name)"].join(", "))
        .eq("user_id", user.id)
        .single();
      if (fetchError || !data) {
        setLoading(false);
  ...
  if (loading || !company) {
    return (
      <div className="max-w-xl mx-auto p-6 bg-white rounded-xl shadow mt-12">
        <div className="flex items-center justify-between mb-4">
          <Skeleton height={32} width={180} radius="md" className="mb-2" />
